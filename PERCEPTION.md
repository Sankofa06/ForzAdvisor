...
- Goal: consolidate the pending ForzAdvisor product and release work through a protected-main review, then deliver the reviewed app to the existing TestFlight Internal group.
...
- Active signals: the new consolidated source has not yet completed its final static, Xcode, UI, and fresh independent review gates. App Store Connect currently has version 1.41.2 build 87 in `VALID` state and associated with Internal; the next candidate will use build 88 after source gates pass. The research pilot's exact install/access route, screenshot fixture, and recruitment/consent approvals remain blocked.
