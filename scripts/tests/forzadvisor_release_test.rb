...
class ForzAdvisorReleaseTest < Minitest::Test
  ...
  def test_repository_release_config_records_verification_only_ci_and_stable_runner
    assert_equal "79", @config.fetch("release", "source_build_number")
    assert_equal "79", @config.fetch("release", "current_app_store_build_number")
  ...
  def test_full_preflight_passes_current_repository_artifacts_with_injected_urls_and_git
    ...
    assert_equal "79", result.dig("checks", "project", "evidence", "source_build_number")
  ...
  def test_project_inspector_checks_versions_signing_schemes_and_both_test_targets
    ...
    assert_equal "79", result["source_build_number"]
  ...
  def test_stable_runner_coordinator_persists_intent_before_exact_upload_and_attaches_only_testflight
    ...
    Dir.mktmpdir do |directory|
      ...
      assert_equal "build-79", state["build_id"]
  ...
  def test_stable_runner_coordinator_rejects_wrong_receipt_and_ambiguous_builds
    ...
    responses["/v1/builds/other/preReleaseVersion"] = responses["/v1/builds/build-79/preReleaseVersion"]
    ...
    assert_raises(ForzAdvisorRelease::APIError) do
      ForzAdvisorRelease::TestFlightDistributor.new(config: @config, api: external_api).call(build_id: "build-79")
    end
  ...
  def test_ambiguous_candidate_reconciliation_is_read_only_and_block_requires_notes
    Dir.mktmpdir do |directory|
      ...
      assert_includes ForzAdvisorRelease::Reporter.text(reconciled), "Build build-79: VALID"
  ...
  def test_terminal_rollover_archives_once_and_establishes_new_github_verified_identity
    Dir.mktmpdir do |directory|
      ...
      store.save(stable_identity.merge("source_build_number" => "78", "schema_version" => 2, "phase" => "human_blocked"))
      data = JSON.parse(File.read(CONFIG_PATH))
  ...
  def test_human_result_records_accept_and_rejects_premature_or_unknown_results
    Dir.mktmpdir do |directory|
      store = ForzAdvisorRelease::StableStateStore.new(directory: directory)
      store.save(stable_identity.merge("schema_version" => 2, "phase" => "human_verification_pending", "build_id" => "build-79"))
  ...
  def test_candidate_validation_rejects_removed_group_wrong_build_prerelease_platform_and_export_value
    group = @config.fetch("testflight", "internal_group", "id")
    mutations = {
      "wrong build" => proc { |responses| responses["/v1/builds/build"]["data"]["attributes"]["version"] = "78" },
  ...
  def test_app_store_status_is_read_only_and_reports_source_and_current_builds
    ...
    responses = {
      ...
      "/v1/apps/#{app_id}/builds" => { "data" => [{ "id" => "build-id", "attributes" => { "version" => "79", "processingState" => "VALID" } }] },
    ...
    assert_equal "79", result["source_build_number"]
    assert_equal "79", result.dig("build", "number")
  ...
  def test_stable_runner_helper_uses_exact_upload_contract_and_parses_only_receipt_line
    ...
    confirmation = "UPLOAD:IOS:#{@config.fetch('app', 'id')}:#{@config.fetch('app', 'bundle_id')}:1.41.1:79:#{commit}"
    observed = helper.upload(commit: commit, app_id: @config.fetch("app", "id"), bundle_id: @config.fetch("app", "bundle_id"), version: "1.41.1", build: "79", confirmation: confirmation)
    ...
    %w[--platform iOS --archive --upload --expected-version 1.41.1 --expected-build 79 --confirm-upload].each do |argument|
  ...
  def stable_candidate_responses
    ...
    group_reads = 0
    {
      ...
      "/v1/builds/build-79" => { "data" => candidate_build },
      "/v1/builds/build-79/app" => { "data" => { "id" => app } },
      "/v1/builds/build-79/preReleaseVersion" => { "data" => { "attributes" => { "version" => "1.41.1", "platform" => "IOS" } } },
      "/v1/betaGroups/#{group}" => { "data" => { "id" => group, "attributes" => { "name" => "Internal", "isInternalGroup" => true } } },
      "/v1/betaGroups/#{group}/app" => { "data" => { "id" => app } },
      "/v1/betaGroups/#{group}/builds" => proc { group_reads += 1; { "data" => group_reads == 1 ? [] : [{ "id" => "build-79" }] } },
  ...
  def candidate_validation_responses(build_id)
    ...
    group = @config.fetch("testflight", "internal_group", "id")
    {
      "/v1/builds/#{build_id}" => { "data" => { "id" => build_id, "attributes" => { "version" => "79", "processingState" => "VALID", "buildAudienceType" => "APP_STORE_ELIGIBLE", "usesNonExemptEncryption" => false } } },
  ...
  def stable_identity
    {
      ...
      "ref" => "release-1.41.1-appstore-79",
      "commit" => "a" * 40
    }
  end
  ...
  def asc_preflight_responses
    ...
    build_attrs = { "version" => "79", "processingState" => "VALID", "buildAudienceType" => "APP_STORE_ELIGIBLE", "usesNonExemptEncryption" => false }
    selected_attrs = build_attrs.merge("version" => "79")
