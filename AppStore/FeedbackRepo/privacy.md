...
Deidentified calibration reuse and JSON sharing are off by default for each experiment. With explicit per-record permission, you can use the iOS system share sheet to share an allow-listed JSON copy. It excludes the local experiment ID, saved tune ID and plan fingerprint, Research Lab record ID and content fingerprint, generated tune values, provider and ruleset data, lap times, telemetry, notes, screenshots, OCR, location, device identifiers, analytics, share destination, and public attribution. It retains a menu-measurement fingerprint to bind the observed controls. ForzAdvisor has no background experiment uploader or importer, and deleting the local record cannot recall a shared copy. Experiments cannot create a ruleset or unlock numeric FH5 tuning.
