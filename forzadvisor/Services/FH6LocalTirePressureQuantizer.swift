...
//  forzadvisor
//
//  Adapts deterministic FH6 formula candidates to first-party, exact-build
//  slider constraints. The legacy tire-only adapter remains for decoding and
//  validating previously generated v1 tunes.
//
...
enum FH6LocalTirePressureRuleset {
    ...
    static func matches(
        ...
        validationStatus: TuneRulesetValidationStatus
    ) -> Bool {
        id == self.id
            && schemaVersion == self.schemaVersion
            && algorithmVersion == self.algorithmVersion
            && knowledgeRevision == self.knowledgeRevision
            && validationStatus == .experimental
    }
}

enum FH6ExactConstraintRuleset {
    static let id = "forzadvisor.fh6.exact-constraint-quantization"
    static let schemaVersion = 1
    static let algorithmVersion = "2.0.0"
    static let knowledgeRevision = "fh6-exact-constraint-capture-v2"
    ...
    static func acceptedEvidenceIDs(
        ...
        guard constraint.scope == .exactVehicleBuild,
              ...
              !constraint.evidenceIDs.isEmpty else {
            return nil
        }
        let evidenceByID = Dictionary(
            uniqueKeysWithValues: snapshot.evidenceSources.map { ($0.id, $0) }
        )
        let evidence = constraint.evidenceIDs.compactMap { evidenceByID[$0] }
        guard evidence.count == constraint.evidenceIDs.count,
              evidence.allSatisfy({ isAccepted($0, in: snapshot) }) else {
            return nil
        }
        return constraint.evidenceIDs
    }

    static func hasAcceptedProvenance(
        _ provenanceIDs: [String],
        in snapshot: VehicleBuildSnapshot
    ) -> Bool {
        guard !provenanceIDs.isEmpty,
              provenanceIDs == Array(Set(provenanceIDs)).sorted() else {
            return false
        }
        let evidenceByID = Dictionary(
            uniqueKeysWithValues: snapshot.evidenceSources.map { ($0.id, $0) }
        )
        return provenanceIDs.allSatisfy { id in
            guard let evidence = evidenceByID[id],
                  isAccepted(evidence, in: snapshot) else {
                return false
            }
            return snapshot.constraints.contains {
                $0.evidenceIDs.contains(id)
    ...
    private static func isAccepted(
        ...
        return recognizedSource
            && evidence.game == .fh6
            && evidence.gameBuildVersion == snapshot.gameBuild.version
...
enum FH6PublicRulesetRegistry {
    static func recognizes(
        id: String,
        schemaVersion: Int,
        algorithmVersion: String,
        knowledgeRevision: String,
        ...
        let legacy = FH6LocalTirePressureRuleset.matches(
            id: id,
            schemaVersion: schemaVersion,
            algorithmVersion: algorithmVersion,
            knowledgeRevision: knowledgeRevision,
        ...
        return legacy || FH6ExactConstraintRuleset.matches(
            id: id,
            schemaVersion: schemaVersion,
            algorithmVersion: algorithmVersion,
            knowledgeRevision: knowledgeRevision,
...
struct FH6ExactConstraintQuantizer {
    func quantize(_ candidate: TuneResult) -> TuneResult {
        guard candidate.request.car.game == .fh6,
              let snapshot = candidate.request.buildSnapshot,
              snapshot.kind == .exactBuildObservation,
        ...
        let fieldCounts = Dictionary(
            grouping: candidate.sections.flatMap(\.lines).compactMap(\.fieldID),
            by: { $0 }
        ).mapValues(\.count)
        let capabilityBySetting = Dictionary(
            uniqueKeysWithValues: TuneCapabilityResolver(game: .fh6)
                .resolve(profile: snapshot.capabilityProfile)
                .settings
                .map { ($0.setting, $0.status) }
        )
        var result = candidate
        var rulesetEvidenceIDs = Set<String>()
        result.sections = candidate.sections.map { section in
            ...
            section.lines = section.lines.map { line in
                guard let field = line.fieldID,
                      field.gearIndex == nil,
                      fieldCounts[field] == 1,
                      capabilityBySetting[field.setting] == .stockAvailable
                        || capabilityBySetting[field.setting] == .installedUpgrade,
                      line.unit == field.expectedDisplayUnit,
                      let rawValue = LocalizedNumberText.parse(line.value),
                      let constraint = snapshot.constraints.first(where: {
                          $0.field == field
                      }),
                      constraint.unit == field.expectedUnit,
                      let evidenceIDs = FH6ExactConstraintRuleset
                        .acceptedEvidenceIDs(for: constraint, in: snapshot),
                      let quantized = quantizedValue(
                          rawValue,
                          using: constraint
                      ) else {
                    return line
                ...
                let capturedDetail =
                    "Aligned to the captured in-game control step."
                if line.detail?.contains(capturedDetail) != true {
                    line.detail = [line.detail, capturedDetail]
                        .compactMap { $0 }
                        .joined(separator: " ")
                }
        ...
        if let reference = FH6ExactConstraintRuleset.reference(
            provenanceIDs: rulesetEvidenceIDs.sorted()
        ) {
            result.rulesetReference = reference
        }
