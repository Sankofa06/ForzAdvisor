...
final class ValidationDraftCleanupCoordinatorTests: XCTestCase {
    ...
    func testGenerationTreatsPostcommitCleanupAsWarningNotFailure() throws {
        let repository = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let source = try String(
            contentsOf: repository.appendingPathComponent(
                "forzadvisor/ContentView+GenerationWorkflow.swift"
            ),
            encoding: .utf8
        )
        let successStart = try XCTUnwrap(source.range(of: "onSuccess: { tune in"))
        ...
        let success = String(source[successStart.lowerBound..<failureStart.lowerBound])
        let failure = String(source[failureStart.lowerBound...])

        XCTAssertTrue(success.contains("ValidationDraftCleanupCoordinator"))
        XCTAssertTrue(success.contains("Tune saved."))
        XCTAssertTrue(success.contains("step = .result"))
        XCTAssertFalse(success.contains("step = .generationFailed"))
        XCTAssertFalse(failure.contains("ValidationPostCommitCleanupError"))
    }
