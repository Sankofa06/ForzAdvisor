...
final class BetaValidationMissionTests: XCTestCase {
    ...
    func testProgressShareIsDeterministicAggregateOnlyAndUsesMarketingURL() {
        ...
        XCTAssertTrue(first.text.contains("Evidence records on this device: 7"))
        XCTAssertTrue(first.text.contains("Local-only records: 7"))
        XCTAssertTrue(first.text.contains("Reusable records: 0"))
